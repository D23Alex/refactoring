  ...
  (let [events (http-get (apis :events)
    ...
    {:game-ids (events :game-ids)
     ...
     :points (+ (* two-pointers-made 2)
                ...
                free-throws-made)
     :assists (count (events :assists))
     :blocks (count (events :blocks))
     :steals (count (events :steals))
     :inbounds (count (events :inbounds))
     :out-of-bounds (count (events :out-of-bounds))
     :fouls (count (events :fouls))
     :turnovers (+ (count (events :steals))
                   (count (events :out_of_bounds))
                   (count (events :blocks))
                   (count (events :rule_violations)))}))
